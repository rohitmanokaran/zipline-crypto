...
class LiveTradingAlgorithm(TradingAlgorithm):
    ...
    def is_date_before_asset_close(self, day: datetime, asset: Asset):
        """
        When we use pipeline live the end date is always yesterday so we add 5
        days to still keep this condition but also allowing to use pipeline
        live as well. 5 is a good number for weekends/holidays
        """
        return day > min(asset.end_date, asset.auto_close_date) + timedelta(days=5)
