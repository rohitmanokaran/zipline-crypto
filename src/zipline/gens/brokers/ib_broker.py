...
from zipline.finance.order import Order as ZPOrder, ORDER_STATUS as ZP_ORDER_STATUS
...
# from ibapi.wrapper import EClientSocket
...
def _method_params_to_dict(args):
    return {k: v for k, v in iteritems(args) if k != "self"}


class TWSConnection(EWrapper):  # EClientSocket, EWrapper
    ...
    def subscribe_to_market_data(
        self, symbol, sec_type="STK", exchange="SMART", currency="USD"
    ):
    ...
    def _process_tick(self, ticker_id, tick_type, value):
        try:
            ...
            log.error(
                "Tick {} for id={} is not registered".format(tick_type, ticker_id)
            )
            return
        if tick_type == 48:
    ...
    def _add_bar(
        ...
        bar = pd.DataFrame(
            index=pd.DatetimeIndex([last_trade_time]),
            data={
    ...
    def execDetailsEnd(self, req_id):
        log.debug(
            "Execution details completed for request {req_id}".format(req_id=req_id)
        )
    ...
    def updateMktDepth(self, ticker_id, position, operation, side, price, size):
        log_message("updateMktDepth", vars())

    def updateMktDepthL2(
        self, ticker_id, position, market_maker, operation, side, price, size
    ):
    ...
    def historicalData(
        self, req_id, date, open_, high, low, close, volume, count, wap, has_gaps
    ):
    ...
    def currentTime(self, time):
        self.time_skew = pd.to_datetime("now", utc=True) - pd.to_datetime(
            long(time), unit="s", utc=True
        )
    ...
    def realtimeBar(self, req_id, time, open_, high, low, close, volume, wap, count):
        value = ";".join(
            [str(close), str(count), str(time), str(volume), str(wap), "true"]
        )
...
class IBBroker(Broker):
    ...
    def subscribe_to_market_data(self, asset):
        if asset not in self.subscribed_assets:
            log.info("Subscribing to market data for {}".format(asset))
            ...
            try:
                ...
                log.warning(
                    "!!!WARNING: I did not manage to subscribe to %s "
                    % str(asset.symbol)
                )
            else:
    ...
    def _get_positions_from_broker(self):
        ...
        for symbol in self._tws.positions:
            ...
            if symbol in self._tws.bars:
                ...
                editable_position._underlying_position.last_sale_date = self._tws.bars[
                    symbol
                ].index.values[-1]
            else:
        ...
        self.metrics_tracker._ledger._portfolio.positions = (
    ...
    def account(self):
        ...
        self.metrics_tracker.override_account_fields(
            ...
            initial_margin_requirement=float(ib_account["FullInitMarginReq"]),
            maintenance_margin_requirement=float(ib_account["FullMaintMarginReq"]),
            available_funds=float(ib_account["AvailableFunds"]),
            excess_liquidity=float(ib_account["ExcessLiquidity"]),
            cushion=float(self._tws.accounts[self.account_id][""]["Cushion"]),
            day_trades_remaining=float(
                self._tws.accounts[self.account_id][""]["DayTradesRemaining"]
            ),
            leverage=float(self._tws.accounts[self.account_id][""]["Leverage-S"]),
            net_leverage=(
                float(ib_account["StockMarketValue"])
                / (
                    float(ib_account["TotalCashValue"])
    ...
    def _parse_order_ref(cls, ib_order_ref):
        if not ib_order_ref or not ib_order_ref.endswith(cls._zl_order_ref_magic):
            return None
        ...
        try:
            ...
            log.warning("Error parsing order metadata: {}".format(ib_order_ref))
            return None
    ...
    def _get_or_create_zp_order(self, ib_order_id, ib_order=None, ib_contract=None):
        ...
        if not order_details and ib_order_id in self._tws.open_orders:
            ...
            order_details = self._parse_order_ref(open_order["order"].m_orderRef)
        ...
        if not asset:
            log.warning(
                ...
                "order but it is not registered in bundle".format(symbol=symbol)
            )
            return None

        if order_details:
            amount = self._action_qty_to_amount(
                order_details["action"], order_details["qty"]
            )
    ...
    def _ib_to_zp_status(ib_status):
        ...
        elif ib_status in ("pendingsubmit", "pendingcancel", "presubmitted"):
    ...
    def _update_orders(self):
                ...
                if zp_status is None:
                    log.warning(
                        ...
                        "unknown order status: {order_status}.".format(
                            order_id=ib_order_id, order_status=open_order_state.m_status
                        )
                    )
                else:
            ...
            if ib_order_id in self._tws.order_statuses:
                ...
                if zp_status:
                    ...
                    log.warning(
                        "Order-{order_id}: "
                        "unknown order status: {order_status}.".format(
                            order_id=ib_order_id, order_status=order_status["status"]
                        )
                    )

        def _update_from_execution(zp_order, ib_order_id):
            if (
                ib_order_id in self._tws.executions
                and ib_order_id not in self._tws.open_orders
            ):
        ...
        all_ib_order_ids = (
            set([e.broker_order_id for e in self._orders.values()])
    ...
    def _update_transactions(self):
        ...
        for ib_order_id, executions in iteritems(self._tws.executions):
            orders = [
                order for order in all_orders if order.broker_order_id == ib_order_id
            ]
            ...
            for exec_id, execution in iteritems(executions):
                ...
                amount = exec_detail.m_shares if is_buy else -1 * exec_detail.m_shares
                tx = Transaction(
                    asset=order.asset,
                    amount=amount,
    ...
    def get_spot_value(self, assets, field, dt, data_frequency):
        ...
        if bars.empty:
            ...
            minute_df = bars.between_time(
                minute_start, minute_end, include_start=True, include_end=True
            )
            if minute_df.empty:
                return np.NaN
            else:
